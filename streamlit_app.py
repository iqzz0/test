...
if df is not None:
        ...
        with cols[col_idx]:
            ...
            with st.container(border=True):
                if os.path.exists(img_path):
                    ...
                    # Menggunakan width="stretch" untuk menggantikan parameter lama agar log bersih
    ...
    st.info("Pastikan file 'submission_v2_final.csv' dan folder 'test' berada di direktori yang sama dengan script ini.")import streamlit as st
import pandas as pd
import os
from PIL import Image
...
if df is not None:
    ...
    st.info("Pastikan file 'submission_v2_final.csv'
            # Extract ID numerik untuk sorting
            df['id_num'] = df['filepath'].apply(lambda x: int(x.split('.')[0]) if str(x).split('.')[0].isdigit() else 0)
            df = df.sort_values('id_num').reset_index(drop=True)
            return df
